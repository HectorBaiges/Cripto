...
if __name__ == "__main__":
    gf.GF_Tables()
    results = GF_product_TvsGF_productP(0x02)
    graphResutlsAsBarChart(results, 0x02)
    results = GF_product_TvsGF_productP(0x03)
    graphResutlsAsBarChart(results, 0x03)
    results = GF_product_TvsGF_productP(0x09)
    graphResutlsAsBarChart(results, 0x09)
    results = GF_product_TvsGF_productP(0x0B)
    graphResutlsAsBarChart(results, 0x0B)
    results = GF_product_TvsGF_productP(0x0D)
    graphResutlsAsBarChart(results, 0x0D)
    results = GF_product_TvsGF_productP(0x0E)
    graphResutlsAsBarChart(results, 0x0E)
